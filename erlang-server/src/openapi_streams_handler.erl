%% basic handler
-module(openapi_streams_handler).
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'CreateBigBlueButtonVideoCall'
    }
) ->
    {[<<"GET">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'UpdateSubscriptions'
    }
) ->
    {[<<"PATCH">>], Req, State};
