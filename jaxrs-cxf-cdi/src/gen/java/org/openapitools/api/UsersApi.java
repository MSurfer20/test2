...
public class UsersApi  {
  ...
  @Inject UsersApiService delegate;
    ...
    public Response createUser( @NotNull @ApiParam(value = "The email address of the new user. ",required=true)  @QueryParam("email") String email,  @NotNull @ApiParam(value = "The password of the new user. ",required=true)  @QueryParam("password") String password,  @NotNull @ApiParam(value = "The full name of the new user. ",required=true)  @QueryParam("full_name") String fullName) {
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Success", response = JsonSuccessBase.class) })
    public Response getSubscriptionStatus(@ApiParam(value = "The target user's ID. ",required=true) @PathParam("user_id") Integer userId, @ApiParam(value = "The ID of the stream to access. ",required=true) @PathParam("stream_id") Integer streamId) {
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Success.", response = JsonSuccessBase.class) })
    public Response getUser(@ApiParam(value = "The target user's ID. ",required=true) @PathParam("user_id") Integer userId, @ApiParam(value = "Whether the client supports computing gravatars URLs.  If enabled, `avatar_url` will be included in the response only if there is a Zulip avatar, and will be `null` for users who are using gravatar as their avatar.  This option significantly reduces the compressed size of user data, since gravatar URLs are long, random strings and thus do not compress well. The `client_gravatar` field is set to `true` if clients can compute their own gravatars. ", defaultValue="false") @DefaultValue("false")  @QueryParam("client_gravatar") Boolean clientGravatar, @ApiParam(value = "Whether the client wants [custom profile field](/help/add-custom-profile-fields) data to be included in the response.  **Changes**: New in Zulip 2.1.0.  Previous versions do no offer these data via the API. ", defaultValue="false") @DefaultValue("false")  @QueryParam("include_custom_profile_fields") Boolean includeCustomProfileFields) {
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Success.", response = JsonSuccessBase.class) })
    public Response getUserByEmail(@ApiParam(value = "The email address of the user whose details you want to fetch. ",required=true) @PathParam("email") String email, @ApiParam(value = "Whether the client supports computing gravatars URLs.  If enabled, `avatar_url` will be included in the response only if there is a Zulip avatar, and will be `null` for users who are using gravatar as their avatar.  This option significantly reduces the compressed size of user data, since gravatar URLs are long, random strings and thus do not compress well. The `client_gravatar` field is set to `true` if clients can compute their own gravatars. ", defaultValue="false") @DefaultValue("false")  @QueryParam("client_gravatar") Boolean clientGravatar, @ApiParam(value = "Whether the client wants [custom profile field](/help/add-custom-profile-fields) data to be included in the response.  **Changes**: New in Zulip 2.1.0.  Previous versions do no offer these data via the API. ", defaultValue="false") @DefaultValue("false")  @QueryParam("include_custom_profile_fields") Boolean includeCustomProfileFields) {
    ...
    public Response muteTopic( @NotNull @ApiParam(value = "The topic to (un)mute. Note that the request will succeed regardless of whether any messages have been sent to the specified topic. ",required=true)  @QueryParam("topic") String topic,  @NotNull @ApiParam(value = "Whether to mute (`add`) or unmute (`remove`) the provided topic. ",required=true, allowableValues="add, remove")  @QueryParam("op") String op, @ApiParam(value = "The name of the stream to access. ")  @QueryParam("stream") String stream, @ApiParam(value = "The ID of the stream to access. ")  @QueryParam("stream_id") Integer streamId) {
    ...
    public Response unsubscribe( @NotNull @ApiParam(value = "A list of stream names to unsubscribe from. This parameter is called `streams` in our Python API. ",required=true)  @QueryParam("subscriptions") List<String> subscriptions, @ApiParam(value = "A list of user ids (preferred) or Zulip display email addresses of the users to be subscribed to or unsubscribed from the streams specified in the `subscriptions` parameter. If not provided, then the requesting user/bot is subscribed.  **Changes**: The integer format is new in Zulip 3.0 (feature level 9). ")  @QueryParam("principals") List<OneOfstringinteger> principals) {
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Success.", response = JsonSuccessBase.class) })
    public Response updateSubscriptions(@ApiParam(value = "A list of stream names to unsubscribe from. ")  @QueryParam("delete") List<String> delete, @ApiParam(value = "A list of objects describing which streams to subscribe to, optionally including per-user subscription parameters (e.g. color) and if the stream is to be created, its description. ")  @QueryParam("add") List<Object> add) {
