...
public class MessagesApi  {
  ...
  @Inject MessagesApiService delegate;
    ...
    public Response checkMessagesMatchNarrow( @NotNull @ApiParam(value = "List of IDs for the messages to check.",required=true)  @QueryParam("msg_ids") List<Integer> msgIds,  @NotNull @ApiParam(value = "A structure defining the narrow to check against. See how to [construct a narrow](/api/construct-narrow).",required=true)  @QueryParam("narrow") List<Object> narrow) {
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Success.", response = JsonSuccessBase.class) })
    public Response getMessages( @NotNull  @Min(0)@ApiParam(value = "The number of messages with IDs less than the anchor to retrieve. ",required=true)  @QueryParam("num_before") Integer numBefore,  @NotNull  @Min(0)@ApiParam(value = "The number of messages with IDs greater than the anchor to retrieve. ",required=true)  @QueryParam("num_after") Integer numAfter, @ApiParam(value = "Integer message ID to anchor fetching of new messages. Supports special string values for when the client wants the server to compute the anchor to use:  * `newest`: The most recent message. * `oldest`: The oldest message. * `first_unread`: The oldest unread message matching the   query, if any; otherwise, the most recent message.  The special values of `'newest'` and `'oldest'` are also supported for anchoring the query at the most recent or oldest messages.  **Changes**: String values are new in Zulip 3.0 (feature level 1).  The   `first_unread` functionality was supported in Zulip 2.1.x   and older by not sending anchor and using use_first_unread_anchor.    In Zulip 2.1.x and older, `oldest` can be emulated with   `anchor=0`, and `newest` with `anchor=10000000000000000`   (that specific large value works around a bug in Zulip   2.1.x and older in the `found_newest` return value). ")  @QueryParam("anchor") OneOfstringinteger anchor, @ApiParam(value = "The narrow where you want to fetch the messages from. See how to [construct a narrow](/api/construct-narrow). ")  @QueryParam("narrow") List<Object> narrow, @ApiParam(value = "Whether the client supports computing gravatars URLs.  If enabled, `avatar_url` will be included in the response only if there is a Zulip avatar, and will be `null` for users who are using gravatar as their avatar.  This option significantly reduces the compressed size of user data, since gravatar URLs are long, random strings and thus do not compress well. The `client_gravatar` field is set to `true` if clients can compute their own gravatars. ", defaultValue="false") @DefaultValue("false")  @QueryParam("client_gravatar") Boolean clientGravatar, @ApiParam(value = "If `true`, message content is returned in the rendered HTML format. If `false`, message content is returned in the raw Markdown-format text that user entered. ", defaultValue="true") @DefaultValue("true")  @QueryParam("apply_markdown") Boolean applyMarkdown, @ApiParam(value = "Legacy way to specify `anchor=\"first_unread\"` in Zulip 2.1.x and older.  Whether to use the (computed by the server) first unread message matching the narrow as the `anchor`.  Mutually exclusive with `anchor`.  **Changes**: Deprecated in Zulip 3.0, replaced by `anchor=\"first_unread\"` instead. ", defaultValue="false") @DefaultValue("false")  @QueryParam("use_first_unread_anchor") Boolean useFirstUnreadAnchor) {
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Success.", response = JsonSuccessBase.class) })
    public Response renderMessage( @NotNull @ApiParam(value = "The content of the message. Maximum message size of 10000 bytes. ",required=true)  @QueryParam("content") String content) {
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Success.", response = JsonSuccess.class),
        @ApiResponse(code = 400, message = "Bad request.", response = CodedError.class) })
    public Response updateMessage(@ApiParam(value = "The target message's ID. ",required=true) @PathParam("message_id") Integer messageId, @ApiParam(value = "The topic to move the message(s) to, to request changing the topic. Should only be sent when changing the topic, and will throw an error if the target message is not a stream message.  Maximum length of 60 characters.  **Changes**: New in Zulip 2.0.  Previous Zulip releases encoded this as `subject`, which is currently a deprecated alias. ")  @QueryParam("topic") String topic, @ApiParam(value = "Which message(s) should be edited: just the one indicated in `message_id`, messages in the same topic that had been sent after this one, or all of them.  Only the default value of `change_one` is valid when editing only the content of a message.  This parameter determines both which messages get moved and also whether clients that are currently narrowed to the topic containing the message should navigate or adjust their compose box recipient to point to the post-edit stream/topic. ", allowableValues="change_one, change_later, change_all", defaultValue="change_one") @DefaultValue("change_one")  @QueryParam("propagate_mode") String propagateMode, @ApiParam(value = "Whether to send breadcrumb message to the old thread to notify users where the messages were moved to.  **Changes**: New in Zulip 3.0 (feature level 9). ", defaultValue="true") @DefaultValue("true")  @QueryParam("send_notification_to_old_thread") Boolean sendNotificationToOldThread, @ApiParam(value = "Whether to send a notification message to the new thread to notify users where the messages came from.  **Changes**: New in Zulip 3.0 (feature level 9). ", defaultValue="true") @DefaultValue("true")  @QueryParam("send_notification_to_new_thread") Boolean sendNotificationToNewThread, @ApiParam(value = "The content of the message. Maximum message size of 10000 bytes. ")  @QueryParam("content") String content, @ApiParam(value = "The stream ID to move the message(s) to, to request moving messages to another stream.  Should only be sent when changing the stream, and will throw an error if the target message is not a stream message. ")  @QueryParam("stream_id") Integer streamId) {
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Success.", response = JsonSuccessBase.class) })
    public Response updateMessageFlags( @NotNull @ApiParam(value = "An array containing the IDs of the target messages. ",required=true)  @QueryParam("messages") List<Integer> messages,  @NotNull @ApiParam(value = "Whether to `add` the flag or `remove` it. ",required=true, allowableValues="add, remove")  @QueryParam("op") String op,  @NotNull @ApiParam(value = "The flag that should be added/removed. ",required=true)  @QueryParam("flag") String flag) {
