...
class Messages {
  ...
  @override
  String toString() => 'Messages[avatarUrl=$avatarUrl, client=$client, content=$content, contentType=$contentType, displayRecipient=$displayRecipient, id=$id, isMeMessage=$isMeMessage, reactions=$reactions, recipientId=$recipientId, senderEmail=$senderEmail, senderFullName=$senderFullName, senderId=$senderId, senderRealmStr=$senderRealmStr, streamId=$streamId, subject=$subject, topicLinks=$topicLinks, submessages=$submessages, timestamp=$timestamp, type=$type]';
  ...
  /// Returns a new [Messages] instance and imports its values from
  /// [json] if it's non-null, null if [json] is null.
  static Messages fromJson(Map<String, dynamic> json) => json == null
    ? null
    : Messages(
  ...
  static List<Messages> listFromJson(List<dynamic> json, {bool emptyIsNull, bool growable,}) =>
    json == null || json.isEmpty
      ? true == emptyIsNull ? null : <Messages>[]
      : json.map((dynamic value) => Messages.fromJson(value)).toList(growable: true == growable);

  static Map<String, Messages> mapFromJson(Map<String, dynamic> json) {
    final map = <String, Messages>{};
    if (json?.isNotEmpty == true) {
      json.forEach((key, value) => map[key] = Messages.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of Messages-objects as value to a dart map
  static Map<String, List<Messages>> mapListFromJson(Map<String, dynamic> json, {bool emptyIsNull, bool growable,}) {
    final map = <String, List<Messages>>{};
    if (json?.isNotEmpty == true) {
      json.forEach((key, value) {
        map[key] = Messages.listFromJson(value, emptyIsNull: emptyIsNull, growable: growable,);
      });
    }
    return map;
  }
