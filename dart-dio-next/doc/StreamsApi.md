...
final api = Openapi().getStreamsApi();
final int streamId = 1; // int | The ID of the stream to access. 
final String topicName = new coffee machine; // String | The name of the topic to delete. 

try { 
    final response = api.deleteTopic(streamId, topicName);
    print(response);
} catch on DioError (e) {
...
try { 
    final response = api.getSubscribers(streamId);
    print(response);
} catch on DioError (e) {
