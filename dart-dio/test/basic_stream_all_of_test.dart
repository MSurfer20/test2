//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.7

// ignore_for_file: unused_import

import 'package:openapi/model/basic_stream_all_of.dart';
import 'package:test/test.dart';

// tests for BasicStreamAllOf
void main() {
  final instance = BasicStreamAllOfBuilder();
  // TODO add properties to the builder and call build()

  group(BasicStreamAllOf, () {
    // JsonObject streamId
    test('to test the property `streamId`', () async {
      // TODO
    });

    // JsonObject name
    test('to test the property `name`', () async {
      // TODO
    });

    // JsonObject description
    test('to test the property `description`', () async {
      // TODO
    });

    // JsonObject dateCreated
    test('to test the property `dateCreated`', () async {
      // TODO
    });

    // JsonObject inviteOnly
    test('to test the property `inviteOnly`', () async {
      // TODO
    });

    // JsonObject renderedDescription
    test('to test the property `renderedDescription`', () async {
      // TODO
    });

    // JsonObject isWebPublic
    test('to test the property `isWebPublic`', () async {
      // TODO
    });

    // JsonObject streamPostPolicy
    test('to test the property `streamPostPolicy`', () async {
      // TODO
    });

    // JsonObject messageRetentionDays
    test('to test the property `messageRetentionDays`', () async {
      // TODO
    });

    // JsonObject historyPublicToSubscribers
    test('to test the property `historyPublicToSubscribers`', () async {
      // TODO
    });

    // JsonObject firstMessageId
    test('to test the property `firstMessageId`', () async {
      // TODO
    });

    // JsonObject isAnnouncementOnly
    test('to test the property `isAnnouncementOnly`', () async {
      // TODO
    });


  });

}
