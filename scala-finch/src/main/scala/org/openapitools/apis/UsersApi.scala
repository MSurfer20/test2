...
import org.openapitools.models.OneOf&lt;object,object,object,object,object,object&gt;
    ...
    */
    ...
    implicit class StringOps(s: String) {
      ...
      def toZonedDateTime: ZonedDateTime = ZonedDateTime.parse(s, datetimeformatter)
        ...
        * @return An endpoint representing a JsonSuccessBase
        */
        private def updateSettings(da: DataAccessor): Endpoint[JsonSuccessBase] =
        patch("settings" :: paramOption("full_name") :: paramOption("email") :: paramOption("old_password") :: paramOption("new_password") :: paramOption("twenty_four_hour_time").map(_.map(_.toBoolean)) :: paramOption("dense_mode").map(_.map(_.toBoolean)) :: paramOption("starred_message_counts").map(_.map(_.toBoolean)) :: paramOption("fluid_layout_width").map(_.map(_.toBoolean)) :: paramOption("high_contrast_mode").map(_.map(_.toBoolean)) :: paramOption("color_scheme").map(_.map(_.toInt)) :: paramOption("enable_drafts_synchronization").map(_.map(_.toBoolean)) :: paramOption("translate_emoticons").map(_.map(_.toBoolean)) :: paramOption("default_language") :: paramOption("default_view") :: paramOption("left_side_userlist").map(_.map(_.toBoolean)) :: paramOption("emojiset") :: paramOption("demote_inactive_streams").map(_.map(_.toInt)) :: paramOption("timezone") :: paramOption("enable_stream_desktop_notifications").map(_.map(_.toBoolean)) :: paramOption("enable_stream_email_notifications").map(_.map(_.toBoolean)) :: paramOption("enable_stream_push_notifications").map(_.map(_.toBoolean)) :: paramOption("enable_stream_audible_notifications").map(_.map(_.toBoolean)) :: paramOption("notification_sound") :: paramOption("enable_desktop_notifications").map(_.map(_.toBoolean)) :: paramOption("enable_sounds").map(_.map(_.toBoolean)) :: paramOption("email_notifications_batching_period_seconds").map(_.map(_.toInt)) :: paramOption("enable_offline_email_notifications").map(_.map(_.toBoolean)) :: paramOption("enable_offline_push_notifications").map(_.map(_.toBoolean)) :: paramOption("enable_online_push_notifications").map(_.map(_.toBoolean)) :: paramOption("enable_digest_emails").map(_.map(_.toBoolean)) :: paramOption("enable_marketing_emails").map(_.map(_.toBoolean)) :: paramOption("enable_login_emails").map(_.map(_.toBoolean)) :: paramOption("message_content_in_email_notifications").map(_.map(_.toBoolean)) :: paramOption("pm_content_in_desktop_notifications").map(_.map(_.toBoolean)) :: paramOption("wildcard_mentions_notify").map(_.map(_.toBoolean)) :: paramOption("desktop_icon_count_display").map(_.map(_.toInt)) :: paramOption("realm_name_in_notifications").map(_.map(_.toBoolean)) :: paramOption("presence_enabled").map(_.map(_.toBoolean)) :: paramOption("enter_sends").map(_.map(_.toBoolean))) { (fullName: Option[String], email: Option[String], oldPassword: Option[String], newPassword: Option[String], twentyFourHourTime: Option[Boolean], denseMode: Option[Boolean], starredMessageCounts: Option[Boolean], fluidLayoutWidth: Option[Boolean], highContrastMode: Option[Boolean], colorScheme: Option[Int], enableDraftsSynchronization: Option[Boolean], translateEmoticons: Option[Boolean], defaultLanguage: Option[String], defaultView: Option[String], leftSideUserlist: Option[Boolean], emojiset: Option[String], demoteInactiveStreams: Option[Int], timezone: Option[String], enableStreamDesktopNotifications: Option[Boolean], enableStreamEmailNotifications: Option[Boolean], enableStreamPushNotifications: Option[Boolean], enableStreamAudibleNotifications: Option[Boolean], notificationSound: Option[String], enableDesktopNotifications: Option[Boolean], enableSounds: Option[Boolean], emailNotificationsBatchingPeriodSeconds: Option[Int], enableOfflineEmailNotifications: Option[Boolean], enableOfflinePushNotifications: Option[Boolean], enableOnlinePushNotifications: Option[Boolean], enableDigestEmails: Option[Boolean], enableMarketingEmails: Option[Boolean], enableLoginEmails: Option[Boolean], messageContentInEmailNotifications: Option[Boolean], pmContentInDesktopNotifications: Option[Boolean], wildcardMentionsNotify: Option[Boolean], desktopIconCountDisplay: Option[Int], realmNameInNotifications: Option[Boolean], presenceEnabled: Option[Boolean], enterSends: Option[Boolean]) =>
          da.Users_updateSettings(fullName, email, oldPassword, newPassword, twentyFourHourTime, denseMode, starredMessageCounts, fluidLayoutWidth, highContrastMode, colorScheme, enableDraftsSynchronization, translateEmoticons, defaultLanguage, defaultView, leftSideUserlist, emojiset, demoteInactiveStreams, timezone, enableStreamDesktopNotifications, enableStreamEmailNotifications, enableStreamPushNotifications, enableStreamAudibleNotifications, notificationSound, enableDesktopNotifications, enableSounds, emailNotificationsBatchingPeriodSeconds, enableOfflineEmailNotifications, enableOfflinePushNotifications, enableOnlinePushNotifications, enableDigestEmails, enableMarketingEmails, enableLoginEmails, messageContentInEmailNotifications, pmContentInDesktopNotifications, wildcardMentionsNotify, desktopIconCountDisplay, realmNameInNotifications, presenceEnabled, enterSends) match {
            case Left(error) => checkError(error)
            case Right(data) => Ok(data)
          }
        } handle {
        ...
        * @return An endpoint representing a JsonSuccess
        */
        private def updateStatus(da: DataAccessor): Endpoint[JsonSuccess] =
        post("users" :: "me" :: "status" :: paramOption("status_text") :: paramOption("away").map(_.map(_.toBoolean)) :: paramOption("emoji_name") :: paramOption("emoji_code") :: paramOption("reaction_type")) { (statusText: Option[String], away: Option[Boolean], emojiName: Option[String], emojiCode: Option[String], reactionType: Option[String]) =>
          da.Users_updateStatus(statusText, away, emojiName, emojiCode, reactionType) match {
            case Left(error) => checkError(error)
            case Right(data) => Ok(data)
          }
        } handle {
