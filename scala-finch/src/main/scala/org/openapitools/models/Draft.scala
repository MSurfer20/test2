 ...
 * @param _type The type of the draft. Either unaddressed (empty string), \"stream\", or \"private\" (for PMs and private group messages). 
 ...
 */
case class Draft(id: Option[Int],
                _type: String,
                to: Seq[Int],
                topic: String,
                content: String,
...
object Draft {
    /**
     * Creates the codec for converting Draft from and to JSON.
     */
    implicit val decoder: Decoder[Draft] = deriveDecoder
    implicit val encoder: ObjectEncoder[Draft] = deriveEncoder
}
