...

{
  ...
  @override
  String toString() {
    return 'GetMessages[avatarUrl=$avatarUrl, client=$client, content=$content, contentType=$contentType, displayRecipient=$displayRecipient, id=$id, isMeMessage=$isMeMessage, reactions=$reactions, recipientId=$recipientId, senderEmail=$senderEmail, senderFullName=$senderFullName, senderId=$senderId, senderRealmStr=$senderRealmStr, streamId=$streamId, subject=$subject, topicLinks=$topicLinks, submessages=$submessages, timestamp=$timestamp, type=$type, flags=$flags, lastEditTimestamp=$lastEditTimestamp, matchContent=$matchContent, matchSubject=$matchSubject, ]';
  }
}

@GenSerializer(nullableFields: true)
class GetMessagesSerializer extends Serializer<GetMessages> with _$GetMessagesSerializer {
