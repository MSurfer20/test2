...
public class MessagesApi  {
   private final MessagesApiService delegate = MessagesApiServiceFactory.getMessagesApi();
    ...
    public Response addReaction(@ApiParam(value = "The target message's ID. ",required=true) @PathParam("message_id") Integer messageId
,@ApiParam(value = "The target emoji's human-readable name.  To find an emoji's name, hover over a message to reveal three icons on the right, then click the smiley face icon. Images of available reaction emojis appear. Hover over the emoji you want, and note that emoji's text name. ",required=true) @QueryParam("emoji_name") String emojiName
,@ApiParam(value = "A unique identifier, defining the specific emoji codepoint requested, within the namespace of the `reaction_type`.  For most API clients, you won't need this, but it's important for Zulip apps to handle rare corner cases when adding/removing votes on an emoji reaction added previously by another user.  If the existing reaction was added when the Zulip server was using a previous version of the emoji data mapping between Unicode codepoints and human-readable names, sending the `emoji_code` in the data for the original reaction allows the Zulip server to correctly interpret your upvote as an upvote rather than a reaction with a \"diffenent\" emoji. ") @QueryParam("emoji_code") String emojiCode
,@ApiParam(value = "If an app is adding/removing a vote on an existing reaction, it should pass this parameter using the value the server provided for the existing reaction for specificity.  Supported values:  * `unicode_emoji`: Unicode emoji (`emoji_code` will be its Unicode codepoint). * `realm_emoji`: Custom emoji. (`emoji_code` will be its ID). * `zulip_extra_emoji`: Special emoji included with Zulip.  Exists to    namespace the `zulip` emoji.  **Changes**: In Zulip 3.0 (feature level 2), this become optional for [custom emoji](/help/add-custom-emoji); previously, this endpoint assumed `unicode_emoji` if this parameter was not specified. ") @QueryParam("reaction_type") String reactionType
)
    throws NotFoundException {
        return delegate.addReaction(messageId,emojiName,emojiCode,reactionType);
    }
    @GET
    @Path("/matches_narrow")
    ...
    public Response checkMessagesMatchNarrow(@ApiParam(value = "List of IDs for the messages to check.",required=true) @QueryParam("msg_ids") List<Integer> msgIds
,@ApiParam(value = "A structure defining the narrow to check against. See how to [construct a narrow](/api/construct-narrow).",required=true) @QueryParam("narrow") List<Object> narrow
)
    throws NotFoundException {
        return delegate.checkMessagesMatchNarrow(msgIds,narrow);
    }
    @DELETE
    @Path("/{message_id}")
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        ...
        @io.swagger.annotations.ApiResponse(code = 400, message = "Bad request.", response = JsonSuccess.class) })
    public Response deleteMessage(@ApiParam(value = "The target message's ID. ",required=true) @PathParam("message_id") Integer messageId
)
    throws NotFoundException {
    ...
    public Response getMessageHistory(@ApiParam(value = "The target message's ID. ",required=true) @PathParam("message_id") Integer messageId
)
    throws NotFoundException {
        return delegate.getMessageHistory(messageId);
    }
    @GET
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "Success.", response = JsonSuccessBase.class) })
    public Response getMessages(@ApiParam(value = "The number of messages with IDs less than the anchor to retrieve. ",required=true) @QueryParam("num_before") Integer numBefore
,@ApiParam(value = "The number of messages with IDs greater than the anchor to retrieve. ",required=true) @QueryParam("num_after") Integer numAfter
,@ApiParam(value = "Integer message ID to anchor fetching of new messages. Supports special string values for when the client wants the server to compute the anchor to use:  * `newest`: The most recent message. * `oldest`: The oldest message. * `first_unread`: The oldest unread message matching the   query, if any; otherwise, the most recent message.  The special values of `'newest'` and `'oldest'` are also supported for anchoring the query at the most recent or oldest messages.  **Changes**: String values are new in Zulip 3.0 (feature level 1).  The   `first_unread` functionality was supported in Zulip 2.1.x   and older by not sending anchor and using use_first_unread_anchor.    In Zulip 2.1.x and older, `oldest` can be emulated with   `anchor=0`, and `newest` with `anchor=10000000000000000`   (that specific large value works around a bug in Zulip   2.1.x and older in the `found_newest` return value). ") @QueryParam("anchor") OneOfstringinteger anchor
,@ApiParam(value = "The narrow where you want to fetch the messages from. See how to [construct a narrow](/api/construct-narrow). ") @QueryParam("narrow") List<Object> narrow
...
,@ApiParam(value = "If `true`, message content is returned in the rendered HTML format. If `false`, message content is returned in the raw Markdown-format text that user entered. ", defaultValue="true") @DefaultValue("true") @QueryParam("apply_markdown") Boolean applyMarkdown
,@ApiParam(value = "Legacy way to specify `anchor=\"first_unread\"` in Zulip 2.1.x and older.  Whether to use the (computed by the server) first unread message matching the narrow as the `anchor`.  Mutually exclusive with `anchor`.  **Changes**: Deprecated in Zulip 3.0, replaced by `anchor=\"first_unread\"` instead. ", defaultValue="false") @DefaultValue("false") @QueryParam("use_first_unread_anchor") Boolean useFirstUnreadAnchor
)
    throws NotFoundException {
        return delegate.getMessages(numBefore,numAfter,anchor,narrow,clientGravatar,applyMarkdown,useFirstUnreadAnchor);
    }
    @GET
    @Path("/{message_id}")
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        ...
        @io.swagger.annotations.ApiResponse(code = 400, message = "Bad request.", response = JsonSuccessBase.class) })
    public Response getRawMessage(@ApiParam(value = "The target message's ID. ",required=true) @PathParam("message_id") Integer messageId
)
    throws NotFoundException {
        return delegate.getRawMessage(messageId);
    }
    @DELETE
    @Path("/{message_id}/reactions")
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        ...
        @io.swagger.annotations.ApiResponse(code = 400, message = "Bad request.", response = JsonSuccess.class) })
    public Response removeReaction(@ApiParam(value = "The target message's ID. ",required=true) @PathParam("message_id") Integer messageId
,@ApiParam(value = "The target emoji's human-readable name.  To find an emoji's name, hover over a message to reveal three icons on the right, then click the smiley face icon. Images of available reaction emojis appear. Hover over the emoji you want, and note that emoji's text name. ") @QueryParam("emoji_name") String emojiName
,@ApiParam(value = "A unique identifier, defining the specific emoji codepoint requested, within the namespace of the `reaction_type`.  For most API clients, you won't need this, but it's important for Zulip apps to handle rare corner cases when adding/removing votes on an emoji reaction added previously by another user.  If the existing reaction was added when the Zulip server was using a previous version of the emoji data mapping between Unicode codepoints and human-readable names, sending the `emoji_code` in the data for the original reaction allows the Zulip server to correctly interpret your upvote as an upvote rather than a reaction with a \"diffenent\" emoji. ") @QueryParam("emoji_code") String emojiCode
,@ApiParam(value = "If an app is adding/removing a vote on an existing reaction, it should pass this parameter using the value the server provided for the existing reaction for specificity.  Supported values:  * `unicode_emoji`: Unicode emoji (`emoji_code` will be its Unicode codepoint). * `realm_emoji`: Custom emoji. (`emoji_code` will be its ID). * `zulip_extra_emoji`: Special emoji included with Zulip.  Exists to    namespace the `zulip` emoji.  **Changes**: In Zulip 3.0 (feature level 2), this become optional for [custom emoji](/help/add-custom-emoji); previously, this endpoint assumed `unicode_emoji` if this parameter was not specified. ") @QueryParam("reaction_type") String reactionType
)
    throws NotFoundException {
        return delegate.removeReaction(messageId,emojiName,emojiCode,reactionType);
    }
    @POST
    @Path("/render")
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "Success.", response = JsonSuccessBase.class) })
    public Response renderMessage(@ApiParam(value = "The content of the message. Maximum message size of 10000 bytes. ",required=true) @QueryParam("content") String content
)
    throws NotFoundException {
        return delegate.renderMessage(content);
    }
    @POST
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        ...
        @io.swagger.annotations.ApiResponse(code = 400, message = "Bad request.", response = JsonSuccessBase.class) })
    public Response sendMessage(@ApiParam(value = "The type of message to be sent. `private` for a private message and `stream` for a stream message. ",required=true, allowableValues="private, stream") @QueryParam("type") String type
,@ApiParam(value = "For stream messages, either the name or integer ID of the stream. For private messages, either a list containing integer user IDs or a list containing string email addresses.  **Changes**: Support for using user/stream IDs was added in Zulip 2.0.0. ",required=true) @QueryParam("to") List<Integer> to
,@ApiParam(value = "The content of the message. Maximum message size of 10000 bytes. ",required=true) @QueryParam("content") String content
,@ApiParam(value = "The topic of the message. Only required for stream messages (`type=\"stream\"`), ignored otherwise.  Maximum length of 60 characters.  **Changes**: New in Zulip 2.0.  Previous Zulip releases encoded this as `subject`, which is currently a deprecated alias. ") @QueryParam("topic") String topic
...
,@ApiParam(value = "For clients supporting local echo, a unique string-format identifier chosen freely by the client; the server will pass it back to the client without inspecting it, as described in the `queue_id` description. ") @QueryParam("local_id") String localId
)
    throws NotFoundException {
        return delegate.sendMessage(type,to,content,topic,queueId,localId);
    }
    @PATCH
    @Path("/{message_id}")
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        ...
        @io.swagger.annotations.ApiResponse(code = 400, message = "Bad request.", response = JsonSuccess.class) })
    public Response updateMessage(@ApiParam(value = "The target message's ID. ",required=true) @PathParam("message_id") Integer messageId
...
,@ApiParam(value = "Which message(s) should be edited: just the one indicated in `message_id`, messages in the same topic that had been sent after this one, or all of them.  Only the default value of `change_one` is valid when editing only the content of a message.  This parameter determines both which messages get moved and also whether clients that are currently narrowed to the topic containing the message should navigate or adjust their compose box recipient to point to the post-edit stream/topic. ", allowableValues="change_one, change_later, change_all", defaultValue="change_one") @DefaultValue("change_one") @QueryParam("propagate_mode") String propagateMode
,@ApiParam(value = "Whether to send breadcrumb message to the old thread to notify users where the messages were moved to.  **Changes**: New in Zulip 3.0 (feature level 9). ", defaultValue="true") @DefaultValue("true") @QueryParam("send_notification_to_old_thread") Boolean sendNotificationToOldThread
,@ApiParam(value = "Whether to send a notification message to the new thread to notify users where the messages came from.  **Changes**: New in Zulip 3.0 (feature level 9). ", defaultValue="true") @DefaultValue("true") @QueryParam("send_notification_to_new_thread") Boolean sendNotificationToNewThread
,@ApiParam(value = "The content of the message. Maximum message size of 10000 bytes. ") @QueryParam("content") String content
,@ApiParam(value = "The stream ID to move the message(s) to, to request moving messages to another stream.  Should only be sent when changing the stream, and will throw an error if the target message is not a stream message. ") @QueryParam("stream_id") Integer streamId
)
    throws NotFoundException {
        return delegate.updateMessage(messageId,topic,propagateMode,sendNotificationToOldThread,sendNotificationToNewThread,content,streamId);
    }
    @POST
    @Path("/flags")
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "Success.", response = JsonSuccessBase.class) })
    public Response updateMessageFlags(@ApiParam(value = "An array containing the IDs of the target messages. ",required=true) @QueryParam("messages") List<Integer> messages
,@ApiParam(value = "Whether to `add` the flag or `remove` it. ",required=true, allowableValues="add, remove") @QueryParam("op") String op
,@ApiParam(value = "The flag that should be added/removed. ",required=true) @QueryParam("flag") String flag
)
    throws NotFoundException {
        return delegate.updateMessageFlags(messages,op,flag);
    }
}
