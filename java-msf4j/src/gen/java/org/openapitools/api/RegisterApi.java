...
public class RegisterApi  {
   private final RegisterApiService delegate = RegisterApiServiceFactory.getRegisterApi();
    ...
    public Response registerQueue(@ApiParam(value = "Set to `true` if you would like the content to be rendered in HTML format (otherwise the API will return the raw text that the user entered) ", defaultValue="false") @DefaultValue("false") @QueryParam("apply_markdown") Boolean applyMarkdown
...
,@ApiParam(value = "Setting this to `true` will make presence dictionaries be keyed by user_id instead of email.  **Changes**: New in Zulip 3.0 (Unstable with no feature level yet). ", defaultValue="false") @DefaultValue("false") @QueryParam("slim_presence") Boolean slimPresence
...
,@ApiParam(value = "A JSON-encoded array of arrays of length 2 indicating the narrow for which you'd like to receive events for. For instance, to receive events for the stream `Denmark`, you would specify `narrow=[['stream', 'Denmark']]`.  Another example is `narrow=[['is', 'private']]` for private messages. Default is `[]`. ") @QueryParam("narrow") List<List<String>> narrow
    ...
    throws NotFoundException {
        return delegate.registerQueue(applyMarkdown,clientGravatar,slimPresence,eventTypes,allPublicStreams,includeSubscribers,clientCapabilities,fetchEventTypes,narrow);
    }
}
